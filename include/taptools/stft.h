...
namespace tap::tools {
    ...
    class basic_stft {
        static_assert(is_sample_profile<Sample>,
                      ...
                      "basic_stft supports the two Tap numeric profiles: float and double");
      ...
      public:
        ...
        void configure(int fftsize, int overlap = k_default_overlap) {
            ...
            m_window.assign(m_fftsize, Sample(0.0));
            for (int k = 0; k < m_fftsize; ++k) {
                m_window[k] = Sample(0.5) - Sample(0.5) * std::cos(Sample(2.0) * k_pi * k / m_fftsize);
            }
            ...
            std::vector<Sample> cola(m_fftsize, Sample(0.0));
            for (int o = 0; o < m_overlap; ++o) {
                for (int k = 0; k < m_fftsize; ++k) {
                    cola[(o * m_hop + k) % m_fftsize] += m_window[k] * m_window[k];
                }
            }
            const Sample c = cola[m_fftsize / 2];
            m_norm         = (c > Sample(0.0)) ? (Sample(1.0) / c) : Sample(1.0);

            m_fft.emplace(static_cast<size_t>(m_fftsize));
            m_spec.assign(m_fftsize, Sample(0.0));
            m_re.assign(m_fftsize, Sample(0.0));
            m_im.assign(m_fftsize, Sample(0.0));
            reset();
        }

        // Flush the running buffers without touching the window/size (safe from a message handler).
        void reset() {
            m_inbuf.assign(m_fftsize, Sample(0.0));
            m_outbuf.assign(m_fftsize, Sample(0.0));
            m_pos      = 0;
            m_hopcount = 0;
        }
        ...
        void process(const Sample* in, Sample* out, long n, SpectralOp&& op) {
            for (long i = 0; i < n; ++i) {
                m_inbuf[m_pos]  = in[i];
                out[i]          = m_outbuf[m_pos];
                m_outbuf[m_pos] = Sample(0.0);
      ...
      private:
        ...
        void process_frame(SpectralOp&& op) {
            ...
            const Sample scale = Sample(2.0) / static_cast<Sample>(N);
            for (int k = 0; k < N; ++k) {
        ...
        Sample m_norm{Sample(1.0)};
        int    m_pos{0};
        int    m_hopcount{0};

        std::optional<tap::dsp::basic_real_fft<Sample>> m_fft;    // sized in configure()
        std::vector<Sample>                             m_window; // Hann (analysis and synthesis)
        std::vector<Sample>                             m_inbuf;  // circular input buffer (length N)
        std::vector<Sample>                             m_outbuf; // circular overlap-add accumulator (length N)
        std::vector<Sample>                             m_spec;   // packed FFT scratch (time in / spectrum / time out)
        std::vector<Sample>                             m_re;     // unpacked half-spectrum real (bins 0..N/2)
        std::vector<Sample>                             m_im;     // unpacked half-spectrum imag (bins 0..N/2)
    };

    /// The double profile — the golden model.
    using stft = basic_stft<double>;

    /// The float profile — for single-precision targets. See numeric.h.
    using stft32 = basic_stft<float>;
