...
public:
    ...
    double process(double in, double d_read, double d_half, double fb, double lp_a, double ap_c, double pickup) {
...
public:
    ...
    void set_freq(int voice, double hz)  { if (valid_voice(voice)) set_param(p_freq1 + voice, hz); }
    void set_res(int voice, double pct)  { if (valid_voice(voice)) set_param(p_res1 + voice, pct); }
    void set_lp(int voice, double hz)    { if (valid_voice(voice)) set_param(p_lp1 + voice, hz); }
...
private:
    ...
    void update_derived() {
        ...
        for (int v = 0; v < k_voices; ++v) {
            const double f = std::clamp(val[p_freq1 + v].current * val[p_freq_master].current,
                                        k_freq_floor_hz, f_ceil);
            ...
            const double fc = std::clamp(val[p_lp1 + v].current * val[p_lp_master].current, 0.0, k_freq_ceil_hz);
