...
inline double clamp_param(int index, double value) {
    switch (index) {
        case p_gain:      return value;
        case p_mix:       return std::clamp(value, 0.0, 100.0);
        case p_window:    return std::clamp(value, k_min_window_ms, k_max_window_ms);
        case p_xfade:     return std::clamp(value, 1.0, 100.0);
        case p_modfreq:   return std::clamp(value, 0.0, 20.0);
        case p_moddepth:  return std::clamp(value, 0.0, k_max_trans_st);
        case p_modphase:  return std::clamp(value, 0.0, 360.0);
        case p_randdepth: return std::clamp(value, 0.0, k_max_trans_st);
        case p_randrate:  return std::clamp(value, 0.0, 50.0);
        case p_trans1:
        case p_trans2:    return std::clamp(value, -k_max_trans_st, k_max_trans_st);
        case p_delay1:
        case p_delay2:    return std::clamp(value, 0.0, k_max_delay_ms);
        case p_fb1:
        case p_fb2:       return std::clamp(value, 0.0, k_fb_max * 100.0);
        case p_gain1:
        case p_gain2:     return std::clamp(value, 0.0, 100.0);
...
public:
    ...
    double process(double in, double ratio, double delay_samples, double window_samples,
                   double flank, double fb) {
...
public:
    ...
    void set_trans(int voice, double st)  { if (valid_voice(voice)) set_param(p_trans1 + voice, st); }
    void set_delay(int voice, double ms)  { if (valid_voice(voice)) set_param(p_delay1 + voice, ms); }
    void set_fb(int voice, double pct)    { if (valid_voice(voice)) set_param(p_fb1 + voice, pct); }
    void set_voice_gain(int voice, double pct) { if (valid_voice(voice)) set_param(p_gain1 + voice, pct); }
    ...
    const params& preset(int slot) const { return m_presets[static_cast<size_t>(std::clamp(slot, 0, k_presets - 1))]; }
...
private:
    ...
    long smooth_samples() const {
        return static_cast<long>(m_smooth_ms * 0.001 * m_sr);
    }
