...
void *adsr_new(t_symbol *msg, short argc, t_atom *argv)
{
    ...
    if(x){
		...
		TTObjectInstantiate(kTTSym_adsr,		&x->adsr,		maxNumChannels);
		TTObjectInstantiate(kTTSym_audiosignal,	&x->audioIn,	1);
		TTObjectInstantiate(kTTSym_audiosignal,	&x->audioOut,	1);
