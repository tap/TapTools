...
void bits_list(t_bits *x, t_symbol *msg, short argc, t_atom *argv)
{
	...
	if(x->mode==ps_bits2ints){						// *** BIT-LIST TO INTEGER(LIST) MODE ***
		...
		for(j=0; j<argc; j++){
			SETLONG(templist+1, j);						// Store the row in the output list
			...
			for(i=0; i < x->matrix_width; i++){
				SETLONG(templist+0, i);						// Store the column in the output list
				SETLONG(templist+2, 1 & temp);				// Store the switch value in the output list
...
void bits_int(t_bits *x, long value)
{
	...
	if(x->mode==ps_ints2bits){
		for(i=0; i < 32; i++){
			SETLONG(x->bitlist+(31-i), 1 & value);
			value = value>>1;
		}
