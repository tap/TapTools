...
typedef struct _filter	{
	...
	double					attrFrequency;				///< ATTRIBUTE: Filter cutoff or center frequency, depending on the kind of filter
	double					attrQ;						///< ATTRIBUTE: Filter resonance
...
int TTCLASSWRAPPERMAX_EXPORT main(void)
{
	...
	CLASS_ATTR_DOUBLE(c,	"frequency",	0,	t_filter, attrFrequency);
	CLASS_ATTR_ACCESSORS(c,	"frequency",	NULL, filter_setFrequency);

	CLASS_ATTR_DOUBLE(c,	"q",	0,	t_filter, attrQ);
	...
	//filter_class = c;
class_register(_sym_box, c); 	filter_class = c;	
...
void filter_gettypes(t_filter *x)
{
	...
	for(TTUInt16 i=0; i<v.getSize(); i++){
		...
		v.get(i, classname);
		atom_setsym(a+1, gensym(classname));
