...
extern "C" int C74_EXPORT main(void)
{
	...
	c = class_new("tap.gang",(method)gang_new, (method)gang_free, sizeof(t_gang), (method)0L, A_GIMME, 0);
	...
	// no inletinfo method, all inlets are hot

class_register(_sym_box, c); 	gang_class = c;
}
...
//	if(x->set_by_pp == false) return;	// this has already been set by pp, which takes precedance
	...
	n = argc;
	TTLimitMax<int>(n, MAX_LIST_LENGTH);
	ap = x->argv;
	x->type_is_list = true;
