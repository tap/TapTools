...
std::vector<double> noise_burst(double burst_s, double total_s, double amp = 0.5) {
    std::vector<double> sig(static_cast<size_t>(total_s * g_sr), 0.0);
    lcg rng;
    const size_t nb = static_cast<size_t>(burst_s * g_sr);
...
int main(int argc, char** argv) {
    ...
    // 1. Defaults: a noise burst through the factory tuning.
    {
        comb_bank bank;
        bank.prepare(g_sr);
        write_wav(dir + "5comb_defaults_burst.wav", render(bank, noise_burst(0.1, 5.0)), g_sr);
    }
    ...
    // 2. High resonance: sparse clicks, long clean rings (the res=100 no-clipper behavior).
    {
        ...
        for (int v = 0; v < k::k_voices; ++v)
            bank.set_res(v, 92.0);
        std::vector<double> sig(static_cast<size_t>(8.0 * g_sr), 0.0);
        for (size_t i = 0; i < sig.size(); i += static_cast<size_t>(2.5 * g_sr))
            sig[i] = 0.8;
        write_wav(dir + "5comb_res92_clicks.wav", render(bank, sig), g_sr);
    }
    ...
    // 3. Master-frequency glide: the per-sample smoothing that the legacy object lacked.
    {
        ...
        write_wav(dir + "5comb_master_freq_glide.wav", render(bank, noise_burst(8.0, 10.0, 0.25)), g_sr);
    }
    ...
    // 4. Preset morph: sustained noise while everything interpolates between two presets.
    {
        ...
        std::vector<double> out(in.size());
        const size_t morph_at = static_cast<size_t>(2.0 * g_sr);
        ...
        write_wav(dir + "5comb_preset_morph.wav", out, g_sr);
    }
    ...
    // 5. Warp then phase: clicks while warp ramps up, then phase sweeps to odd-harmonics-only.
    {
        ...
        for (size_t i = 0; i < in.size(); i += static_cast<size_t>(1.5 * g_sr))
            in[i] = 0.8;
        std::vector<double> out(in.size());
        ...
        write_wav(dir + "5comb_warp_then_phase.wav", out, g_sr);
    }
