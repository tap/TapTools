...
int main(int argc, char** argv) {
    ...
    // 1. Dual shadows: two delayed harmony voices around a plucked line.
    {
        ...
        bank.set_mix(55.0);
        std::vector<double> in(static_cast<size_t>(8.0 * g_sr), 0.0);
        const double notes[] = { 220.0, 277.18, 329.63, 440.0 };
        for (int i = 0; i < 4; ++i)
            pluck(in, 0.4 + 1.6 * i, notes[i]);
        write_wav(dir + "pitchaccum_dual_shadows.wav", render(bank, in), g_sr);
    }
    ...
    // 2. The signature: accumulating feedback spiral (each pass +7 semitones).
    {
        ...
        write_wav(dir + "pitchaccum_spiral_up.wav", render(bank, in), g_sr);
    }
    ...
    // 3. Downward spiral with random modulation — the darker GRM texture.
    {
        ...
        write_wav(dir + "pitchaccum_spiral_down_rand.wav", render(bank, in), g_sr);
    }
    ...
    // 4. LFO modulation tour: vibrato-to-seasick sweep of moddepth while a tone sustains.
    {
        ...
        write_wav(dir + "pitchaccum_lfo_tour.wav", render(bank, in), g_sr);
    }
    ...
    // 5. Preset morph: clean harmony -> wild accumulation, morphed over 4 s mid-audio.
    {
        ...
        std::vector<double> in(static_cast<size_t>(14.0 * g_sr), 0.0);
        for (int i = 0; i < 8; ++i)
            pluck(in, 0.4 + 1.5 * i, i % 2 ? 293.66 : 196.0, 0.5, 0.6);
        std::vector<double> out(in.size());
        const size_t morph_at = static_cast<size_t>(4.0 * g_sr);
        ...
        write_wav(dir + "pitchaccum_preset_morph.wav", out, g_sr);
    }
